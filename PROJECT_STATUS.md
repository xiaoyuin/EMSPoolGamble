...
**EMS Pool Gamble** 是一个功能完整的多人台球计分 Web 应用，经过 v1.6.0 特殊胜利高亮系统与计分优化，实现了完整的特殊胜利玩家高亮显示、计分记录时间排序和界面优化，达到了现代化产品级的用户体验和功能完整性。
...
- **计分记录时间排序**：game.html 和 session_detail.html 的计分详情按时间降序显示（最新记录在最上）
- **数据库查询优化**：修改 `get_session_records` 方法排序逻辑，提升用户查看体验
- **历史页面胜率显示优化**：胜率文字颜色调整为中性灰色，降低视觉干扰，突出主要信息

### 🏆 v1.5.1 特殊胜利高亮系统完善（2025-06-30）
- **特殊胜利高亮功能**：完整实现小金/大金胜利者的高亮显示系统
- **数据库层批量查询**：新增批量查询特殊胜利记录的方法，提升性能
- **全站高亮覆盖**：所有页面的玩家名根据特殊胜利记录动态高亮
- **高亮样式设计**：小金橙色效果，大金紫金色渐变效果，视觉层次分明
- **player_detail 页面增强**：新增小金/大金胜利次数统计，对手名支持点击跳转
- **数据结构优化**：支持多对手记录的链接生成和显示

### 🔒 v1.5.0 安全系统与UI/UX升级（2025-06-28）
- **CSRF 漏洞修复**：修复删除场次操作的CSRF安全漏洞，实现完整的安全防护
- **游戏页面UI重构**：卡片布局优化，快速添加玩家功能升级，支持批量操作
- **胜率智能显示**：有效胜率计算（排除1分对局），新玩家标识
- **交互体验优化**：弹窗模式新建玩家，多种快捷键支持，误操作防护
